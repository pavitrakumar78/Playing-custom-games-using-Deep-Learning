# -*- coding: utf-8 -*-
"""
Created on Sat Jan 16 22:39:44 2016
...
class Statistics:
  def __init__(self, agent, net, mem, env):
    ...
    self.csv_name = "flappy_stats.csv"
    if self.csv_name:
      logger.info("Results are written to %s" % self.csv_name)
  ...
  def on_train(self, cost):
    self.average_cost += (cost - self.average_cost) / float(50000)
  ...
  def write(self, epoch, phase):
    ...
    if self.csv_name:
      if self.validation_states is not None:
        qvalues = self.net.predict(self.validation_states[0,...])
        #print qvalues
