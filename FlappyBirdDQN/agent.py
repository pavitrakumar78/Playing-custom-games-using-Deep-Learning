...
class Agent:
	...
	def step(self, exploration_rate,play=False):
		...
		#current_score is the actual score displayed on the screen!
		...
		if terminal:
			#write to log file only when training
			if not self.env.play:
				a = datetime.datetime.now().replace(microsecond=0)
				print_stats = "Time: "+str(a)+"	Total moves made in a game: "+str(self.total_moves)+"		Total Score: "+str(int(round(self.current_score)))
				#print print_stats
				log = open("log_stuff.txt","a")
				log.write(print_stats+"\n")
				log.close()
			self.total_score_all_games += self.current_score
			self.total_score = 0
			self.total_moves = 0
			self.current_score = 0
	...
	def play(self, num_games):
		...
		for i in xrange(num_games):
			...
			while not self.step(self.exploration_rate_test,play=True):
				pass
			#print self.total_score_all_games/(i+1)
			#print self.total_score_all_games
		log = open("score_log_stuff.txt","a")
		log.write("Total score over all games:"+str(self.total_score_all_games)+"\n")
		log.write("Average score:"+str(self.total_score_all_games/num_games)+"\n")
		log.close()
